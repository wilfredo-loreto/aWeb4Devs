  ...
  render() {
    ...
    if (!this.props.isDefinition) {
      ...
      <a className={styles.link}>
        <img className={styles.itemLogo} src={"/img/" + this.props.img} alt={this.props.title + " logo"}/>
        <div className={styles.titleTagContainer}>
    <h4 className={styles.itemTitle}>{this.props.title}</h4>
    <span className={styles.itemTag}>{this.capitalizeFirstLetter(this.props.tag)}</span>
        </div>
      </a>
    ...
    /* Standard structure for items in the panel */  
      ...
      <a className={styles.linkDefinition}>
        <img className={styles.arrowDecoration} src="/icons/arrowLeft.svg" alt=""/>
    <h4 className={styles.itemTitle}>Definition</h4>
        <img className={styles.arrowDecoration} src="/icons/arrowRight.svg" alt=""/>
      </a>
